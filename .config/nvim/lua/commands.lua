...
cmd(
  'command! LspDiagPrev lua vim.diagnostic.goto_prev({ float = { source = "always", width = 55 }})'
)
cmd(
  'command! LspDiagNext lua vim.diagnostic.goto_next({ float = { source = "always", width = 55 }})'
)
cmd(
  'command! LspDiagLine lua vim.diagnostic.open_float(0, { source = "always", scope = "line", header = false, width = 55 })'
)
