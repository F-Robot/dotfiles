...
local function open_rename_window()
	...
	buf_map(
		"i",
		"<CR>",
		('<Cmd>stopinsert | lua require("utils/mappings/normal/rename").rename_symbol_from_window(%d, "%s")<CR>'):format(
			window,
			current_name
		),
		{
...
local function rename_symbol_from_window(window, current_name)
	...
	if #new_name > 0 and new_name ~= current_name then
		...
		lsp.buf_request(0, lsp_method, params, function(err, result, ctx, config)
			if err then
				vim.notify(("Error running lsp query '%s': %s"):format(lsp_method, err), vim.log.levels.ERROR)
			end
